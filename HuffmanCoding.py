...
class HuffmanCoding:
    ...
    def compress_img(self, file_ext=".txt", shape=0):
        ...
        with open(self.path, "rb") as FileHandler, open(output_path, "wb") as output:
            output.write(
                bytes(
                    (" ".join("{0:08b}".format(ord(x), "b") for x in file_extension)),
                    "latin1",
                )
            )
    ...
    def compress(self, file_ext=".txt", shape=0):
        ...
        output_path = filename + ".bin"
        file_extension = file_ext
        with open(self.path, "r+", encoding="latin1") as file, open(
            output_path, "wb"
        ) as output:
            output.write(
                bytes(
    ...
    def decompress(self):
        ...
        with open(self.path, "rb") as file, open(
            output_path, "w", encoding="latin1"
        ) as output:
            bit_string = ""
            byte = file.read(1)
            shape = 0
